...
def add_sub_layer(sub_layer_path, root_layer):
    ...
    if sub_layer:
        ...
        print(f"Failed to open sub-layer at {sub_layer_path}")
...
def create_root(
    ...
    root_xform = UsdGeom.Xform.Define(stage, f'/{asset_name}')
    Usd.ModelAPI(root_xform).SetKind("component")

    # Create variant sets
    variant_sets = root_xform.GetPrim().GetVariantSets()
    geo_variant_set = variant_sets.AddVariantSet('geo')
    mtl_variant_set = variant_sets.AddVariantSet('shd')

    # Add default variants
    geo_variant_set.AddVariant('default')
    mtl_variant_set.AddVariant('default')

    # Activate 'geo' default variant and define the scope within it
    geo_variant_set.SetVariantSelection('default')
    with geo_variant_set.GetVariantEditContext():
        geo_scope = UsdGeom.Scope.Define(stage, f'/{asset_name}/geo')
        geo_scope.GetPrim().CreateAttribute('purpose', Sdf.ValueTypeNames.Token).Set('default')

    # Activate 'geo' default variant and define the scope within it
    geo_variant_set.SetVariantSelection('default')
    with geo_variant_set.GetVariantEditContext():
        geo_scope = UsdGeom.Scope.Define(stage, f'/{asset_name}/mtl')
        geo_scope.GetPrim().CreateAttribute('purpose', Sdf.ValueTypeNames.Token).Set('default')
